...
-- ----------------------------------------------------------------
USE `yiibaidb`; --点在大部分情况下都可以省略，除非碰到MySQL的关键字
-- ----------------------------------------------------------------
...
SET FOREIGN_KEY_CHECKS=0; --在当前运行中设置全局变量

-- ----------------------------
-- Table structure for `customers`
-- ----------------------------
DROP TABLE IF EXISTS `customers`;--如果有这个表就删除，不存在就忽略
-- -------------------------
--每一条SQL语句都是分号结束
-- -------------------------
CREATE TABLE `customers` (
  `customerNumber` int(11) NOT NULL,--字段名 类型是int（长度），约束（NOT NULL）————不允许为空，
  `customerName` varchar(50) NOT NULL,--可变字符串类型
  `contactLastName` varchar(50) NOT NULL,
  `contactFirstName` varchar(50) NOT NULL,
  -- ----------------------------------------------------
  `phone` varchar(50) NOT NULL,--varchar电话号码并不合适
  -- ----------------------------------------------------
  `addressLine1` varchar(50) NOT NULL,
  `addressLine2` varchar(50) DEFAULT NULL,--可以是NULL
  ...
  -- -------------------------------------------------------
  `creditLimit` decimal(10,2) DEFAULT NULL,-- 总长度是十位，能保留两位小数
  -- -------------------------------------------------------
 
  PRIMARY KEY (`customerNumber`), --进一步约束————主键为customer number
  -----------------------------------------------------------------
  --主键：在一个表中，唯一标识一条记录的字段，例：身份证号码，是不会发生重复的
  --1.主键是一点不能重复的
  --自增长：自动增长
  --2.主键是不是一定自增长？（不是）
  --3.主键一定是int类型吗？（不是）
  --4.主键是不是不能更新？（不是），但必须保证更新后的主键依然是唯一的
  -----------------------------------------------------------------
  KEY `salesRepEmployeeNumber` (`salesRepEmployeeNumber`),
  CONSTRAINT `customers_ibfk_1` FOREIGN KEY (`salesRepEmployeeNumber`) REFERENCES `employees` (`employeeNumber`)--外键名为customers ibfk 1 ，本表中的salesRepEmployeeNumber关联着`employees` (`employeeNumber`) 外键表的名字（外键表的字段名）

) ENGINE=InnoDB DEFAULT CHARSET=utf8;--指定存储引擎为innoDB

-- ----------------------------
-- Records of customers

--增C（create） 删D（delete） 改U（update） 查R（retrieve）
--insert 按照声明插入数据
